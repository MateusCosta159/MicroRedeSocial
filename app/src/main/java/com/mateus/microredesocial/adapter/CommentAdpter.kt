...
import com.mateus.microredesocial.databinding.ItemCommentBinding
...
class CommentAdapter(
    ...
    private fun applyUser(binding: ItemCommentBinding, user: User) {
        ...
        if (!user.photo.isNullOrEmpty()) {
            binding.imgCommentAvatar.setImageBitmap(converter.stringToBitmap(user.photo))
        } else {
    ...
    fun setComments(list: List<Comment>) { comments.clear(); comments.addAll(list); notifyDataSetChanged() }
    fun addComment(comment: Comment) { comments.add(comment); notifyItemInserted(comments.size - 1) }
    fun removeComment(comment: Comment) {
        val idx = comments.indexOfFirst { it.id == comment.id }
        if (idx >= 0) { comments.removeAt(idx); notifyItemRemoved(idx) }
    }
